...
def run_api_server(started_event: mp.Event = None, run_mode: str = None):
    # 使用精简版API服务器
    try:
        from server.simple_api import create_simple_app
        app = create_simple_app()
        logger.info("使用精简版API服务器")
    except ImportError:
        # 如果精简版不存在，回退到原版
        from server.api import create_app
        app = create_app(run_mode=run_mode)
        logger.info("使用原版API服务器")
...
def dump_server_info(after_start=False, args=None):
    ...
    print("\n")
    print("=" * 30 + "数智岐黄 Configuration" + "=" * 30)
    print(f"操作系统：{platform.platform()}")
    print(f"Python版本：{sys.version}")
    print(f"项目版本：{VERSION}")
    print(f"当前Embedding模型：{EMBEDDING_MODEL} @ {embedding_device()}")
    print("使用外部VLLM API，无需启动本地模型服务")
    print("\n")

    if after_start:
        print("服务端运行信息：")
        if args and args.api:
            print(f"    数智岐黄 API Server: {api_address()}")
        if args and args.webui:
            print(f"    数智岐黄 WebUI Server: {webui_address()}")
    print("=" * 30 + "数智岐黄 Configuration" + "=" * 30)
    print("\n")
...
async def start_main_server():
    ...
    if args.all_webui:
        # 精简版：只启动API和WebUI，不启动FastChat服务
    ...
    # Model Worker服务已禁用 - 使用外部VLLM API
    ...
    if process_count() == 0:
        ...
        try:
            ...
            # 精简版：等待进程结束，不支持动态模型切换
            # 动态模型切换功能已禁用，使用外部VLLM API
            for name, process in processes.items():
                if name not in ["model_worker", "online_api"]:
                    if isinstance(process, dict):
                        for work_process in process.values():
                            work_process.join()
                    else:
                        process.join()
